...
router.route('/profile').put(validationRules.userUpdate, validate, catchErr(UserController.updateUser));
...
router.route('/login').post(apiLimiter, validationRules.userLogin, validate, UserController.authenticate);
router.route('/register').post(apiLimiter, validationRules.userRegistration, validate, catchErr(UserController.register));
